...
import org.wahlzeit.model.BridgePhotoFactory;
import org.wahlzeit.model.BridgePhotoManager;
...
public abstract class ModelMain extends AbstractMain {
	...
	protected void startUp(String rootDir) throws Exception {
		...
		log.config(LogBuilder.createSystemMessage().addAction("init BridgePhotoFactory").toString());
		BridgePhotoFactory.initialize();

		log.config(LogBuilder.createSystemMessage().addAction("load BridgePhotos").toString());
		BridgePhotoManager.getInstance().init();
	}
